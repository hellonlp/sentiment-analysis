...
from sentiment_analysis_albert_emoji import modeling
from sentiment_analysis_albert_emoji import optimization
from sentiment_analysis_albert_emoji import tokenization
from sentiment_analysis_albert_emoji.utils import load_csv
from sentiment_analysis_albert_emoji.utils import get_emoji
...
def emoji2id(string_emoji):
    """
    将emoji转为id形式，设置sequence_length=hp.sequence_length
    """
    length = len(string_emoji)
    if length <= hp.sequence_length_emoji:
        return [hp.vocab_emoji_char2id.get(l,0) for l in string_emoji]+[0]*(hp.sequence_length_emoji-length)
    elif length > hp.sequence_length_emoji:
        return [hp.vocab_emoji_char2id.get(l,0) for l in string_emoji[:hp.sequence_length_emoji]]  
...
def get_features_emoji():
    # Load train data
    lines = read_csv(os.path.join(hp.data_dir, hp.train_data))
    # Get features emoji
    contents = [l[1] for l in lines]
    contents_emoji = [get_emoji(l) for l in contents]
    return [emoji2id(l) for l in contents_emoji]
 
    
def get_features_emoji_test():
    # Load train data
    lines = read_csv(os.path.join(hp.data_dir, hp.test_data))
    # Get features emoji
    contents = [l[1] for l in lines]
    contents_emoji = [get_emoji(l) for l in contents]
    return [emoji2id(l) for l in contents_emoji]
...
def get_feature_emoji_test(sentence):
    # Get feature emoji
    content_emoji = get_emoji(sentence) 
    return emoji2id(content_emoji)
...
if __name__ == '__main__':
    ...
    print(feature)
    sentence = "⏲⌚豌豆🥳🥳🥳射手🥰🦰"
    feature = get_feature_emoji_test(sentence)
    print(feature)
