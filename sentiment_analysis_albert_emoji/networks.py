# -*- coding: utf-8 -*-
"""
Created on Thu May 30 20:44:42 2020
...
class NetworkAlbert(object):
    def __init__(self,is_training):
        ...
        # Emoji placeholder
        self.input_emojis = tf.placeholder(tf.int32, shape=[None, hp.sequence_length_emoji], name='input_id_emoji')                    
        
        # Cell emoji embedding
        self.emoji_embedding = cell_embedding(self.input_emojis)
                
        # Concat
        output_all = tf.concat([output_layer_init,self.emoji_embedding],1)
                              
        # Cell textcnn-emoji
        output_layer = cell_textcnn_two(output_all,self.is_training)          
        ...
        with tf.name_scope("Full-connection"):                       
            ...
            # Logit           
            self.logits = tf.nn.bias_add(tf.matmul(output_layer, output_weights, transpose_b=True), output_bias)
        ...
        with tf.variable_scope("Loss"):                        
            ...
            if ckpt and tf.gfile.Exists(ckpt.model_checkpoint_path + checkpoint_suffix):
                ...
                # Load BERT Pre-training LM
                print('First time load BERT model!')
