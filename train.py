    ...
    for i in range(hp.num_train_epochs):
        ...
        for j in range(num_batchs-1):
            ...
            if j%(num_batchs//hp.num_saved_per_epoch)==0:
                saver.save(sess, os.path.join(pwd, 'model', 'model_%s_%s.ckpt'%(str(i),str(j))))
            ...
            if j % hp.print_step == 0:
                ...
                print('Time:%s, Epoch:%s, Batch number:%s/%s, Loss:%s'%(time_now_string(),str(i),str(num_batchs),str(j),str(loss)))           
                #  Loss of Test data
                ...
                print('Time:%s, Epoch:%s, Batch number:%s/%s, Loss(test):%s'%(time_now_string(),str(i),str(num_batchs),str(j),str(loss)))           
